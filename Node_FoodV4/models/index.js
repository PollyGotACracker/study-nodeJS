...
// 현재 프로젝트 폴더 경로를 __dirname 변수에 할당
const __dirname = path.resolve();

// 현재 사용하는 프로젝트 환경을 env.NODE_ENV 변수를 참조하여 가져와라
// 만약 그 변수가 설정되어 있지 않다면 development 문자열로 설정하라
// 환경변수. config 파일 참조
const env = process.env.NODE_ENV || "development";
// const config = require(__dirname + "/../config/config.json")[env];

// config.js 파일의 변수를 전부 가져와라
import configJS from "../config/config.js";

// config.js 파일의 변수 중에 env(development) 변수만 가져와서 config 에 할당
