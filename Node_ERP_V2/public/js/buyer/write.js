document.addEventListener("DOMContentLoaded", () => {
  ...
  btnInput?.addEventListener("click", () => {
    ...
    for (input of buyerInputs) {
      ...
      /**
       * input 항목이 많을 경우
       * 유효성 검사가 필요한 항목과 그렇지 않은 항목이 같이 있다.
       * 이럴 때 유효성 검사가 필요한 항목에 대해서만
  ...
  btnCodeCheck?.addEventListener("click", () => {
    ...
    /**
    1. fetch 함수가 실행되고 web 은 작업 수행
       fetch 가 즉시 "완료" 처리를 해버리기 때문에
    2. fetch 는 서버로부터 응답을 기다리기
    3. 서버로부터 응답이 오면 then() 함수를 실행한다.
       이때 서버에서 전달된  response 정보를 res 변수에 담아서 then() 함수에 전달한다.
  ...
  if (public_bcode) {
    ...
    btnCodeCheck.disabled = "disabled"; // true
