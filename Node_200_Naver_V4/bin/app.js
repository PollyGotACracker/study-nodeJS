...
/**
 * session 을 저장, 관리할 DB store 설정
 */
// express session 의 Strore 를 sessionSequelize 에게 보내서 클래스 생성하기
const SessionStore = sessionSequelize(session.Store);
// sessionStore 생성하기
const sessionStore = new SessionStore({
  db: DB.sequelize,
  expiration: 1000 * 60, // 1분간 유지하기
  // 만료된(expired) 세션을 DB 로부터 자동삭제하는 간격
...
app.use(
  session({
    ...
    store: sessionStore, // session 보관 장소 지정
    cookie: {
      maxAge: 1000 * 60, // store expiration 과 같은 시각 설정
    },
  })
);
...
// 이 설정은 반드시 router 위에 정의할 것
app.use((req, res, next) => {
  if (req?.session?.user) {
    res.locals.user = req.session.user;
  }
  next(); // 라우터로 req 정보 전달해주기
});
