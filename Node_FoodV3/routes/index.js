...
router.get("/", async (req, res, next) => {
   ...
   * 데이터를 SELECT 하고
   * SELECT 가 완전하게 완료되면
   * 다음 res.render() 가 실행되는 것을 보장한다.
   */

  /**
   * mysqlConn.execute() 함수가 실행되었을 때
   * 만약 SQL 오류, 서버접속 오류가 발생하면
   * exception(예외상황)
   *  = MySQL 서버에서 데이터가 정상적으로 도착할 것으로 생각했는데
   *  어떤 문제가 발생하여 그렇지 못하다 하면
   *  이 상황에서 "exception" 이라는 event 가 발생한다.
   * exception event 는 System 에서 발생하는 것으로 if문 등으로 처리를 할 수 없다.
   * 일반적으로 exception 이 발생하면 코드가 실행을 멈추고 메시지를 Console 에 보여준다.
   * 애플리케이션을 사용하는 사용자 입장에서는 Console 메시지를 확인할 수 없고
   ...
   * exception event handling 코드를 제공해주고 있다.
   * try { exception event 가 발생할 가능성이 있는 코드들 }
   * catch(event) { exception event 가 발생하면 처리할 코드들 }
